...
\pagestyle{fancy}
\fancyhf{}
\lhead{\fontsize{10pt}{12pt}\selectfont 
  Le secret bancaire suisse au XXe siècle dans le
  \textit{Journal de Genève} et la \textit{Gazette de Lausanne}
}
\rhead{}
\rfoot{\fontsize{10pt}{12pt}\selectfont \thepage}
\lfoot{\textit{\fontsize{10pt}{12pt}\selectfont 
...
\begin{titlepage} 
	...
	\rule{1pt}{0.94\textheight} % Vertical line
...
Ainsi, nous analysons le taux d'articles du secret bancaire issus 
d'agences de presse et le comparons aux taux du corpus financier.
...
dépêches venant de l'étranger à celle de l'Agence télégraphique suisse
...
En isolant et classant les articles contenant ``secret bancaire'' 
avec la méthode de Reinert nous remarquons qu'entre les deux
...
l'article fortement vers le groupe trois qui contient des termes plutôt
...
D'autres tests similaires pointent vers d'autres divisions, où les
...
Du côté de la \emph{Gazette de Lausanne}, nous trouvons six groupes. Là ou le
\emph{JDG} semble avoir des classes qui sont basées sur des sujets différents
(économie, affaires judiciaires, légal), dans la \emph{GDL} il semble que les
événements marquants de la période génèrent plus d'attention. Car, on retrouve
une classe avec des mots rappelant des affaires judiciaires. Dans cette classe,
on retrouve des termes tels que ``renseignement'', ``tribunal'',
``violer''\ldots{} Cela semble indiquer que les différents scandales entourant
le secret bancaire sont perçus comme plus importants dans la \emph{GDL} que le
\emph{JDG}. Cependant, ici comme dans le \emph{JDG} le nom des banques suisses
apparaît plutôt dans le groupe d'articles référençant des termes financiers plus
généraux: la classe 3, qui est très similaire à la classe 3 dans l'analyse du
\emph{JDG} (avec les noms de devises et des quantités).
...
Cette analyse du sujet montre clairement l'adhésion des deux
...
L'analyse linguistique nous montre que la \emph{Gazette de Lausanne} donne plus
d'importance aux affaires concrètes et le \emph{Journal de Genève} plus au coté
